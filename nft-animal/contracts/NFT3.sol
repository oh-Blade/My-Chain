...
contract NFT is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => bool) isCapital;
    uint TotalSupply;
    bool isRevealed;
    ...
    function granToCapital(uint tokenId) public onlyRole(UPGRADER_ROLE) {
        require(isCapital[tokenId] == false,"already Capital");
        isCapital[tokenId] = true;
    }
    //revoking the nft capital role 
    function revokeToCapital(uint tokenId) public onlyRole(UPGRADER_ROLE) {
        require(isCapital[tokenId] == true,"no Capital");
        isCapital[tokenId] = false;
    }
        ...
        returns (string memory)
    {
        if(isRevealed){
            ...
            return isCapital[tokenId]?
            string(abi.encodePacked(baseURI,tokenId.toString(),baseExtension)):
            string(abi.encodePacked(baseURI,(tokenId + 100000).toString(),baseExtension));
        }else {
